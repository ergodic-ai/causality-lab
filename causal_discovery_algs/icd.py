...
class LearnStructICD(LearnStructBase):
    def __init__(
        ...
        self.test_cond_ancestor = (
            True  # requires nodes in the conditioning set to be possible ancestors
        )
        ...
        self.conditioning_set = {
            self.edge_key(*edge): set() for edge in combinations(nodes_set, 2)
        }
    ...
    def learn_structure(self) -> None:
        ...
        # reset state
        self._state = dict(
            done=False, cond_set_size=0  # Latest iteration result
        )  # next iteration r-value: desired search-radius & conditioning-set-size
    ...
    def learn_structure_iteration(self):
        ...
        if self._state["done"]:
            raise ValueError("ICD algorithm has already concluded")
        ...
        if self._state["cond_set_size"] == 0:  # empty conditioning set
            ...
            self._state["done"] = self._learn_struct_incremental_step(
                self._state["cond_set_size"]
            )
        ...
        self._state[
            "cond_set_size"
        ] += 1  # for the next iteration: increase r (radius & conditioning-set-size)

        return (
            self._state["done"],
            r_value,
        )  # return r-value of latest iteration, i.e., self.graph is r-representing
    ...
    def _pre_calc_conditioning(self, cond_set_size):
        for node_i, node_j in combinations(self.graph.nodes_set, 2):
            if self.graph.is_connected(node_i, node_j):
                self.conditioning_set[self.edge_key(node_i, node_j)] = (
                    self._get_pdsep_range_sets(node_i, node_j, cond_set_size)
                )
    ...
    def _learn_struct_incremental_step(self, cond_set_size=None):
        ...
        for node_i, node_j in combinations(source_pag.nodes_set, 2):
            ...
            for cond in cond_sets:
                ...
                cond_set = cond[
                    0
                ]  # get the set of nodes (in [1] there is the sum-of-minimal-distances)
        ...
        if not done:  # re-orient the skeleton only if it was modified
            ...
            if self.is_selection_bias:
                self.graph.maximally_orient_pattern(
                    rules_set=[5, 6, 7]
                )  # when selection-bias may be present
            if self.is_tail_completeness:
                self.graph.maximally_orient_pattern(
                    rules_set=[8, 9, 10]
                )  # for tail-completeness
    ...
    def _get_pdsep_range_sets(self, node_i, node_j, cond_set_size):
        ...
        pds_tree_i, possible_d_sep_i = create_pds_tree(
            self.graph, node_i, max_depth=cond_set_size
        )
        pds_tree_j, possible_d_sep_j = create_pds_tree(
            self.graph, node_j, max_depth=cond_set_size
        )
        ...
        cond_sets_list_init = pds_tree_i.get_subsets_list(
            set_nodes=possible_d_sep_i, subset_size=cond_set_size
        )
        cond_sets_list_init += pds_tree_j.get_subsets_list(
            set_nodes=possible_d_sep_j, subset_size=cond_set_size
        )
    ...
    def _is_cond_set_possible_ancestor(self, cond_set, node_i, node_j):
        ...
        for z in cond_set:
            if not (
                (
                    self.graph.is_possible_ancestor(
                        ancestor_node=z, descendant_node=node_i
                    )
                )
                or (
                    self.graph.is_possible_ancestor(
                        ancestor_node=z, descendant_node=node_j
                    )
                )
            ):
                return False
...
def create_pds_tree(source_pag, node_root, en_nodes=None, max_depth=None):
    ...
    while len(second_nodes) > 0:
        ...
        for node_3 in neighbors:
            if source_pag.is_possible_collider(
                ...
                added_branch = node_2_tree.get_child_branch(
                    node_3
                )  # get the added child branch
